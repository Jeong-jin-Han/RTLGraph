...
// What the two sides are compared as. Case, the kind of quote mark, and how much
// whitespace sits between words are all things a copy out of a PDF changes
...
const fold = (text: string): string => FOLD.reduce((s, [re, to]) => s.replace(re, to), text).toLowerCase()
...
function flatten(pieces: TextPiece[]): Flat {
  ...
  for (const [index, piece] of pieces.entries()) {
    const folded = fold(piece.str)
    // pdf.js hands back a piece per run, and a run does not have to end with a
    // space; a sentence broken across lines needs one to read as two words.
    const spaced = text.length > 0 && !text.endsWith(' ') && !folded.startsWith(' ')
    if (spaced && folded.length > 0) {
      text += ' '
      from.push(index)
      at.push(0)
    }
    for (let i = 0; i < folded.length; i++) {
      text += folded[i]
      from.push(index)
      // The fold can change a piece's length (a ligature becomes two letters),
      // so this is a hint for slicing, clamped when it is used.
      at.push(Math.min(i, Math.max(piece.str.length - 1, 0)))
    }
  }
  return { text, from, at }
}
...
function locate(page: string, quote: string): { start: number; end: number; score: number } | undefined {
...
export function findQuote(pieces: TextPiece[], quote: string): Highlight | undefined {
  ...
  const found = locate(flat.text, wanted)
