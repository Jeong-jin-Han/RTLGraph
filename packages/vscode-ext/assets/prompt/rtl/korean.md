...
- 기존 코드를 3층 구조로 바꾸는 리팩터라면, 코드를 건드리기 전에 테스트벤치로 기준선
  ...
  리팩터를 섞지 마.
- 새로 만드는 거라면 main component 폴더(comb/data_path, comb/control_path, seq,
  tb)를 먼저 만들고, 데이터패스 상자와 제어 신호 표를 정한 다음 코드를 채워줘.
...
마지막으로 Workflow A 로 RTLGraph 파일도 만들고 node PROJECT_FOLDER/.agent/
