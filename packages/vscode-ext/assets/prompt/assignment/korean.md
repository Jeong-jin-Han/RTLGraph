...
스켈레톤은 못 고치지만 그 주위에 쓰는 건 네 몫이야. 테스트벤치를 만들어줘 —
코드 옆에 `tb_<무엇>.v`, 핸드아웃에 이미 tb/ 가 있으면 거기에. 주어진 포트만
주어진 이름 그대로 구동하고, 설계 자신이 아니라 **핸드아웃**을 기준으로 증명해야 해:
...
바꾸지 않도록. `.agent/ENVIRONMENT.md` 에 있는 시뮬레이터로 전부 돌리고, 무엇이
통과했는지 알려줘.
...
- 그리고 그 일을 하는 줄을 가리키는 `"type": "code"` 의 `links` 항목 —
  검사 쪽(`tb_uart_corner.v:73-80`) 하나, 그 검사가 건드리는 RTL 쪽
