...
| X4 | Export 는 **현재 보이는 뷰**(필터 적용, 잘라냄)를 `<그래프 폴더>/out-<이름>/<이름>.<뷰>.<형식>` 에 쓴다. 폴더는 설정 `rtlgraph.export.folder`(기본 `out-${name}`) | NodeGraph 의 `.이름-imgs` 는 JSON 이 참조하는 **입력 자산**이라 숨겼지만, export 는 꺼내 쓰는 **결과물**이라 보이는 폴더가 찾기 쉽다. 숨김을 원하면 `.${name}-export`. 그래프 폴더 밖(`..`, 절대경로)은 거부 |
...
| A7 | 여러 컴포넌트면 컴포넌트마다 파일 하나. 시스템 파일은 아직 없음 | 시스템 뷰는 M6.5 |
